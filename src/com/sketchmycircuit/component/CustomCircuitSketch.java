...
public class CustomCircuitSketch extends View implements
	...
	/*
	 * 1 stroke = erase + circuit line+resistor+inductor+export 2 stroke= 3
	 * stroke=diode 4 stroke = capacitor.
	 */
	public CustomCircuitSketch(Context context, int width, int height) {
		...
		gestureLib1Stroke = GestureLibraries.fromRawResource(context,
				R.raw.gestures_1);
		if (!gestureLib1Stroke.load()) {

		}
		gestureLib4Stroke = GestureLibraries.fromRawResource(context,
				R.raw.gestures_4);
		if (!gestureLib4Stroke.load()) {

		}
		gestureLib3Stroke = GestureLibraries.fromRawResource(context,
				R.raw.gestures_3);
	...
	public void onGesturePerformed(GestureOverlayView arg0, Gesture gesture) {
		...
		if (bestPrediction.score > 0) {
			...
			if (bestPrediction.name.contains("delete")) {
				...
				alertDialogBuilder
						.setMessage(
								"Do you want to Export this Circuit Sketch? ")
						.setCancelable(false)
				...
				/*
				 * Toast.makeText(context, bestPrediction.name + " " +
				 * bestPrediction.score, Toast.LENGTH_SHORT).show();
				 */
	...
	public Prediction findBestPrediction(ArrayList<Prediction> predictions) {
		...
		Log.d(PROD, bestPrediction.name + " gesture with score "
		...
		if (bestPrediction.score < 10 && bestPrediction.name.contains("wire")) {
			Log.d(PROD, bestPrediction.name
					+ " gesture +++++++++++ with score " + bestPrediction.score);
			name = "resistor";
		} else if (bestPrediction.score > 20
				&& bestPrediction.name.contains("wire"))
			name = "wire";
